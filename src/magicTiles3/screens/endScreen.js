...
export class EndScreen extends UIScreen {
  ...
  addSongItem(index) {
    ...
    item.on(SongItemEvent.Selected, (id) => {
      ...
      this.emit(EndScreenEvent.ItemListSelected, id);
    });
  ...
  onItemBought(id) {
    this.emit(EndScreenEvent.ItemListBought, id);
  }
  ...
  _initBottom() {
    ...
    let texture = Texture.from("spr_btn_back");
    this.buttonHome = new PureSprite(texture, new PureTransform({
    ...
    this.buttonHome.x = GameResizer.width / 6;
    this.buttonHome.displayObject.scale.set(2, 2);
    this.bottom.addChild(this.buttonHome.displayObject);

    Util.registerOnPointerDown(this.buttonHome.displayObject, () => {
      this._onBackHome();
    });
    let textureRetry = Texture.from("spr_btn1");
    let pTransform = new PureTransform({
      alignment  : Alignment.MIDDLE_CENTER,
      usePercent : true,
    });
    ...
    this.buttonReplay = new PureSprite(textureRetry, pTransform, lTransform);
    this.buttonReplay.displayObject.scale.set(1.5, 1.5);
    this.bottom.addChild(this.buttonReplay.displayObject);

    let textureIconRetry = Texture.from("spr_btn_replay");
    this.iconReplay = new PureSprite(textureIconRetry, new PureTransform({
      alignment  : Alignment.MIDDLE_CENTER,
      usePercent : true,
    }), new PureTransform({
      alignment  : Alignment.MIDDLE_CENTER,
    ...
    this.iconReplay.displayObject.scale.set(1.5, 1.5);
    this.bottom.addChild(this.iconReplay.displayObject);
    Util.registerOnPointerDown(this.buttonReplay.displayObject, () => {
      this._onRetry();
    });
  }
  ...
  _onRetry() {
    this.unFillAllStars();
    this.emit(EndScreenEvent.ItemListSelected, this);
  }
  ...
  _onResizeBottom() {
    ...
    if (GameResizer.isLandScape()) {
      this.bottom.y = GameResizer.height / 2.5;
      this.buttonHome.x = GameResizer.width / 5;
    }
  }
}
